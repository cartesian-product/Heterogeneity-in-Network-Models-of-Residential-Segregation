...
def mp_simulate(n):
    ...
    for k in range(iters):
        ...
        initialize(topology_modifier = topology_modifier, excess_housing=0.15, method=method, tolerance_threshold=0.6786040486602921)
        result = Result()
    ...
    with open(f"sch_results_htols_{n}.pkl", "wb") as dill_file:
    ...
    for k in range(iters):
        ...
        method = "schelling"
        topology_modifier = n * 32
        initialize(topology_modifier = topology_modifier, excess_housing=0.15, method=method, tolerance_threshold=0.11370901329234828)
        result = Result()
        result.parameters = [f"Topology Modifier: {topology_modifier}", f"Excess Housing Fraction: {excess_housing_fraction}", f"Method: {method}"]
        update_result()
        for j in range(4000):
    ...
    with open(f"sch_results_ltols_{n}.pkl", "wb") as dill_file:
        dill.dump(results, dill_file)