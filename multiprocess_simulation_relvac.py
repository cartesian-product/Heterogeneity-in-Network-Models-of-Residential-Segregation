...
def update():
    ...
    if object_a.tolerance_threshold == None:
        ...
        if len(candidate_vacancies) == 0:
            ...
            weights = [G.degree(v)/sum([G.degree(v) for v in candidate_vacancies]) for v in candidate_vacancies]
        node_b = candidate_vacancies[np.random.choice(len(candidate_vacancies), p=weights)]
...
def mp_simulate(n):
    ...
    with open(f"sch_results_relvac{n}.pkl", "wb") as dill_file:
    ...
    with open(f"xz_results_relvac{n}.pkl", "wb") as dill_file:
        dill.dump(results, dill_file)